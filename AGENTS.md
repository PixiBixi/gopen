...
Module: `github.com/PixiBixi/gopen` | Go 1.27 | No external deps (stdlib only)
...
| `git.go` | `repoContext` struct, `resolvePath()`, `getRepoContext()` (dispatches to the fast path with a fallback), `repoContextViaGit()` (subprocess fallback, four `git` invocations) |
| `gitfile.go` | Pure-Go `.git` reader, the fast path: `configEntry`, `parseGitConfig()`, `firstConfigValue()`, `lastConfigValue()`, `readConfigFile()`, `branchFromHEAD()`, `isHexSHA()`, `isValidBranchName()`, `discoverRepoLayout()`, `needsGitFallback()`, `readRepoContextFromDisk()`, plus the `configScanner` machinery that resolves `include`/`includeIf` directives |
| `url.go` | `provider` struct, `providers` slice, `buildWebURL()`, `detectProvider()`, `pathJoin()`, `convertToHTTPS()`, line anchor helpers |
...
**Two known gaps in the fast path** (documented in the README, evident in `gitfile.go`): the system-wide git config path is compiled into the `git` binary and can only be guessed (`systemConfigPaths()` covers the standard locations and the one implied by `git` on `PATH`), and the `.git` discovery walk does not stop at a filesystem boundary the way `git` does without `GIT_DISCOVERY_ACROSS_FILESYSTEM`. Both are accepted trade-offs, not bugs to fix silently.
...
- **Other checks**: `govulncheck.yml`, `go-format.yml` (goimports via reviewdog), `markdownlint.yml`, `github-actions.yml` (zizmor workflow security scan), `codeql.yml`, `dependency-review.yml` (PR-only, fails on high-severity or GPL/AGPL deps), `scorecard.yml` (OpenSSF Scorecard), `validate-pr-title.yml` (enforces Conventional Commits on the PR title, since a squash merge takes that title as the commit message `svu` reads)
- **Release** (`.github/workflows/release.yml`): automatic on push to `main`, [`svu`](https://github.com/caarlos0/svu) computes the next `vX.Y.Z` from the conventional commits since the last tag, the workflow creates the tag through the API, then GoReleaser builds multi-platform binaries + updates the Homebrew tap in the same job. Only `feat:` (minor) and `fix:` (patch) cut a release; **`perf:` does not**: svu follows the Conventional Commits spec, where only those two are normative. `--v0` keeps a breaking change from jumping to `v1.0.0` while pre-1.0. Renovate drives dep releases (gomod minor → `feat(deps)`, patch/digest → `fix(deps)`, github-actions → `chore(deps)` = no release). Manual `v*` tag push still works. A separate `verify` job then re-downloads the published artifacts and checks the cosign signature and build provenance attestation.
- **GoReleaser config**: `.goreleaser.yml`, also builds the `ghcr.io/pixibixi/gopen` container image via its `kos` (ko) section; there is no Dockerfile in the repo.
